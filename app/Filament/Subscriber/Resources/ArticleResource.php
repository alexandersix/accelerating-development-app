...
use App\Filament\Subscriber\Resources\ArticleResource\Pages;
...
class ArticleResource extends Resource
{
    ...
    {
        return $table
            ->modifyQueryUsing(fn (Builder $query) => self::getModifiedQuery($query)->orderByDesc('created_at'))
            ->columns([
     ...
     * articles belonging to publications which the
     * subscriber user is already subscribed to.
     */
    protected static function getModifiedQuery(Builder $query): Builder
    {
        return $query->whereHas('publication', function (Builder $query) {
            $query->whereHas('subscribers', function (Builder $query) {
